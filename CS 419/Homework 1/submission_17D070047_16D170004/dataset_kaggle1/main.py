...
parser.add_argument("--min_leaf_size", type=int, nargs="?", default=1)
parser.add_argument("--absolute", nargs='?')
parser.add_argument("--mean_squared", nargs='?')

args = parser.parse_args()

if args.mean_squared is None:
...
MAX_DEPTH = 500
MIN_NODE_LENGTH = args.min_leaf_size
data.head()
...
class Node:
    ...
    def assignAttr(self):
        ...
        if self.attributeIndex == -1:
            ...
            self.threshold, self.infGain = splitValueSelection(
                self.X[:, self.attributeIndex], self.y)
...
def makeDecisionTree(node):
    ...
    split_index = np.searchsorted(
        X[:, bestAttribute], node.threshold, side='right') - 1
...
def prune_condition(node, error_metric='mse'):
    ...
    if parent.data_val.shape[0] == 0:
        ...
        print('yooo')
        infGain = parentError - (parent.leftChild.data_val.shape[0] * leftError + (
...
print('Before pruning',error(y, X))
...
X_test = np.array(pd.read_csv(args.test_data))
